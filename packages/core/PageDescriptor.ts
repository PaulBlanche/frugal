...
export interface NoDataResult extends BaseResult {
    /** a http status */
    status: NoDataStatus;
}

export interface WithDataResult<DATA> extends BaseResult {
    /** data object passed to the vue */
    data?: DATA;
}

export type DataResult<DATA = unknown> =
    | RedirectResult
    | NoDataResult
    | WithDataResult<DATA>;
...
export type RawHandler<PATH extends string = string> = (
    context: DynamicDataContext<PATH>,
...
const baseDescriptorSchema = zod.object({
    ...
    POST: zod.optional(zod.function(zod.tuple([]), zod.any(), {
        invalid_type_error:
            'Expected a page descriptor with "POST" as a function',
    })),
    PUT: zod.optional(zod.function(zod.tuple([]), zod.any(), {
        invalid_type_error:
            'Expected a page descriptor with "PUT" as a function',
    })),
    PATCH: zod.optional(zod.function(zod.tuple([]), zod.any(), {
        invalid_type_error:
            'Expected a page descriptor with "PATCH" as a function',
    })),
...
const dynamicDescriptorSchema: zod.Schema<DynamicPageDescriptor> =
    baseDescriptorSchema.extend({
        type: zod.literal('dynamic', {
            required_error:
                'A dynamic page descriptor must have a string "type"',
            invalid_type_error:
...
const staticDescriptorSchema: zod.Schema<StaticPageDescriptor> =
    baseDescriptorSchema.extend({
        type: zod.literal('static', {
            required_error:
                'A static page descriptor must have a string "type"',
            invalid_type_error:
                'Expected a static page descriptor with "type" as "static"',
        }),
