...
export async function regenerate(request: RegenerationRequest, configPath:string): Promise<boolean> {
    logger().info({
        op: 'start',
        url: request.url,
        msg() {
    ...
    const success = await new Promise<boolean>((resolve, reject) => {
        ...
        worker.addEventListener('message', (message: MessageEvent<OutboundMessage>) => {
...
self.addEventListener('message', async (message: MessageEvent<InboundMessage>) => {
    if (message.data.type === 'regenerate') {
        try {
            ...
            const frugal = await Frugal.load(configModule[configModuleExports[0]])
            const success = await frugal.handleRegenerate(message.data.request)
