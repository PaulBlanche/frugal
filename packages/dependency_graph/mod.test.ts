...
Deno.test("dependency_graph/custom resolution/loading", async () => {
  ...
  asserts.assertEquals(
    await Promise.all(asSpy(load).calls.map(async (call) => ({ params: call.params[0].toString(), result: await call.result }))),
    [
      { params: "file:///entrypoint1.ts", result: undefined },
      { params: "file:///foo.ts", result: undefined },
      { params: "virtual:///bar.ts", result: "\n        // bar.ts\n    " },
    ]
  )
...
function module(
  ...
  const dependencies = (moduleConfig.dependencies ?? []).map((dependency) => {
    ...
    return module(ffs, { ...dependency, entrypoint: moduleConfig.entrypoint });
  });
