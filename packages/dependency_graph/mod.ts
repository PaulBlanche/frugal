...
type Config = {
  load?: (resolvedModuleSpecifier: URL) => Promise<string|undefined>|undefined,
...
export async function build(entrypoints: URL[], config: Config = {}): Promise<tree.Root> {
  ...
  async function buildForEntrypoint(resolvedEntrypoint: URL) {
    ...
    while ((current = queue.pop()) !== undefined) {
      if (current.type === "precursor") {
        ...
        if (current.parent !== undefined) {
          const parentAlreadyHasThisDependency = current.parent.dependencies
            .some(
              (dependency) => dependency.url.toString() === module.url.toString(),
            );
        ...
        current.moduleHash = current.dependencies.reduce((hash, dependency) => {
          return hash.update(dependency.moduleHash);
        }, new murmur.Hash().update(current.moduleHash)).alphabetic();
      }
    }
    ...
    async function generateModule(
      ...
      const { dependencies, contentHash } = await analyzeMemoized(precursor.url);
...
async function analyze(resolvedModuleSpecifier: URL, config: Config): Promise<AnalysisResult> {
  ...
  function handleImport(specifier: string) {  
    const resolvedDependency = resolveModule(specifier, resolvedModuleSpecifier, config.resolve)
...
function resolveModule(specifier: string, referrer: URL, resolve: Config['resolve']) {
