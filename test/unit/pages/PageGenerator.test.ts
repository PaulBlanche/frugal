...
import { DataResponse, EmptyResponse } from '../../../src/page/FrugalResponse.ts';
...
Deno.test('PageGenerator: generate orchestrate the generation of DynamicPage', async (t) => {
    ...
    const store: Record<string, { data?: Data; headers?: HeadersInit; status?: number }> = {
        '345': {
    ...
    const pageGetContentStub = mock.stub(page, 'getContent', ({ data: { content } }) => content);
    ...
    const generatorRenderSpy = mock.stub(generator, 'render', originalGeneratorRender);
    ...
    await t.step('data response', async () => {
        ...
        asserts.assertEquals(Array.from(result.headers.entries()), store['345'].headers);
    ...
    await t.step('empty response', async () => {
        ...
        asserts.assertEquals(Array.from(result.headers.entries()), store['325'].headers);
