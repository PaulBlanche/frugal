...
export function TocNavigation({ toc }: TocNavigationProps) {
    ...
    return (
        <nav class={cx(s.tocNavigation)}>
            ...
            {nextNode && nextNode.slug && (
                <TocNavigationLink type='next' node={nextNode} />
            )}
        </nav>
    );
}
...
function TocNavigationLink({ type, node }: TocNavigationLinkProps) {
    return (
        ...
        >
            {type === 'previous' && (
                <Carret class={cx(s.carret)} height='20px' type='left' />
            )}
            {nodeFullLabel(node)}
            {type === 'next' && (
                <Carret class={cx(s.carret)} height='20px' type='right' />
            )}
        </a>
    );
}
