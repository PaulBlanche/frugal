...
export function cssModule(
      ...
      build.register({
        ...
        setup(build) {
          ...
          build.onLoad({ filter }, async (args) => {
            ...
            log(
              `found css module entrypoint "${utils.name(specifier, config)}"`,
              {
                kind: 'debug',
      ...
      build.register({
        ...
        setup(build) {
          ...
          build.onLoad({ filter }, async (args) => {
            ...
            return {
              ...loaded,
              contents: cssModuleCache.get(specifier),
              loader: 'js',
            };
          });
        },
  ...
  function transformCssModule(
    ...
    const { code, exports = {} } = lightning.transform({
      filename: name,
      code: typeof cssCode === 'string'
        ? TEXT_ENCODER.encode(cssCode)
        : cssCode,
      cssModules: true,
    });
    ...
    cssModuleContent.push(
      config.isDevMode
        ? `export default wrap(names, moduleName);`
...
function id(str?: string | undefined): string | undefined {
  ...
  return `${
    str?.split(/\W+/).map((word) => `${word[0].toUpperCase()}${word.slice(1)}`)
      .join('')
  }_${hashCode(str)}`;
}
