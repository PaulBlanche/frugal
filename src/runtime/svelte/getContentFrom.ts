...
const DEFAULT_DOCUMENT: Document = (head, html) =>
  `<!DOCTYPE html><html><head>${head}</head></body>${html}</body>`;
...
export function getContentFrom(
  ...
  return ({ descriptor, assets, data, pathname }) => {
    ...
    if (embedData) {
      const script = `<script>window.__FRUGAL__ = ${
