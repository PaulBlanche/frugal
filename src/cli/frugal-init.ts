...
if (!isDirEmpty(root)) {
  if (
    flags.force === false ||
    flags.force === undefined && !confirm(CONFIRM_FORCE_MESSAGE)
  ) {
    Deno.exit(1);
  }
}
...
const tasks = [
  createFromTemplate(path.join(root, 'cli'), 'dev.ts', 'dev.ts.template'),
  createFromTemplate(path.join(root, 'cli'), 'build.ts', 'build.ts.template'),
  createFromTemplate(path.join(root, 'cli'), 'serve.ts', 'serve.ts.template'),
  createFromTemplate(
    path.join(root, 'cli'),
    'entrypoint.ts',
    'entrypoint.ts.template',
  ),
];
...
async function createDenoConfig(root: string) {
  const denoConfig = {
    ...
    tasks: {
      ...
      build: 'deno run -A --unstable cli/build.ts',
      serve: 'deno run -A --unstable cli/serve.ts',
    },
  };
...
async function createFromTemplate(
  ...
  const dotenvAbsoluteURL = new URL(
    import.meta.resolve('../../dep/std/dotenv.ts'),
  );
