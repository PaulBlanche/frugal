...
> = CONTEXT["rest"] extends `${infer CHAR}${infer REST}` ? CONTEXT["escape"] extends true ? Consume<
...
> = ConsumeModifier<ConsumePattern<CONTEXT>> extends {
    ...
    : never
    : CONTEXT["rest"] extends `(${infer REST}` ? { object: CONTEXT["object"]; index: CONTEXT["index"]; rest: REST }
...
> = INPUT["rest"] extends `(${infer _PATTERN})${infer REST}` ? INPUT["name"] extends string ? {
            name: INPUT["name"];
            rest: REST;
            index: INPUT["index"];
        }
...
type ConsumeParameterName<INPUT extends ParameterContextBase> = INPUT["rest"] extends `:${infer REST}` ? {
        index: INPUT["index"];
...
type ConsumeWord<INPUT extends string, WORD extends string = ""> = Lowercase<INPUT> extends `${infer CHAR}${infer REST}`
