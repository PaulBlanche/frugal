...
type ConsumeParameterName<INPUT extends ParameterContextBase> = INPUT['rest'] extends
    `:${infer REST}` ? {
        index: INPUT['index'];
...
type ConsumeWord<INPUT extends string, WORD extends string = ''> = Lowercase<INPUT> extends
    `${infer CHAR}${infer REST}` ? CHAR extends 'a' ? ConsumeWord<REST, `${WORD}${CHAR}`>
