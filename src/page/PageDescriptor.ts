...
export interface DynamicDataContext<PATH extends string = string> extends StaticDataContext<PATH> {
...
export type GetContentContext<DATA = unknown, PATH extends string = string> = {
    ...
    assets: Partial<Record<string, any>>;
};
...
const baseDescriptorSchema = zod.object({
    ...
    getContent: zod.function(zod.tuple([]), zod.any(), {
        ...
        invalid_type_error: 'Expected a page descriptor with "getContent" as a function',
    }),
    ...
    OPTIONS: zod.optional(zod.function(zod.tuple([]), zod.any(), {
        invalid_type_error: 'Expected a page descriptor with "OPTIONS" as a function',
    })),
...
const dynamicDescriptorSchema: zod.Schema<DynamicPageDescriptor> = baseDescriptorSchema.extend({
    type: zod.literal('dynamic', {
        required_error: 'A dynamic page descriptor must have a string "type"',
        invalid_type_error: 'Expected a dynamic page descriptor with "type" as "dynamic"',
    }),
...
const staticDescriptorSchema: zod.Schema<StaticPageDescriptor> = baseDescriptorSchema.extend({
    getPathList: zod.optional(zod.function(zod.tuple([]), zod.any(), {
        invalid_type_error: 'Expected a static page descriptor with "getPathList" as a function',
    })),
    GET: zod.optional(zod.function(zod.tuple([]), zod.any(), {
        invalid_type_error: 'Expected a static page descriptor with "GET" as a function',
    })),
});
